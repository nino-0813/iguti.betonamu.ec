...
export class GeminiService {
  ...
  constructor() {
    this.apiKey = import.meta.env.VITE_GEMINI_API_KEY || import.meta.env.GEMINI_API_KEY || null;
    if (!this.apiKey) {
  ...
  initChat(products: Product[]) {
    if (!this.ai) {
      console.warn('Gemini API is not available. Chat functionality disabled.');
      return;
    }
